...
void FPropertyWindow::SetTarget(const FVector &Location, const FVector &Rotation, const FVector &Scale,
...
bool FPropertyWindow::DrawVector3Control(const char *Label, const FVector &Value, FVector &OutValue, float Speed,
                                         const char *Format)
{
...
void FPropertyWindow::DrawDecalComponentDetails(UDecalComponent* DecalComponent, FEditorEngine* Engine)
{
    ...
    // Material 선택
    FMaterial* CurrentMaterial = DecalComponent->GetDecalMaterial();
    const char* CurrentName = CurrentMaterial ? CurrentMaterial->GetOriginName().c_str() : "None";

    if (ImGui::BeginCombo("Decal Material", CurrentName))
    {
        TArray<FString> MaterialNames = FMaterialManager::Get().GetAllMaterialNames();
        for (const FString& MaterialName : MaterialNames)
        {
            bool bSelected = (CurrentMaterial && CurrentMaterial->GetOriginName() == MaterialName);
            if (ImGui::Selectable(MaterialName.c_str(), bSelected))
            {
                ...
                {
                    DecalComponent->SetDecalMaterial(Material.get());
                }
            }
        }
        ImGui::EndCombo();
...
void FPropertyWindow::DrawBillboardComponentDetails(UBillboardComponent *BillboardComponent, FEditorEngine *Engine)
{
...
void FPropertyWindow::DrawDetailsSection(UActorComponent *Component, FEditorEngine *Engine)
{
    ...
    {
        DrawBillboardComponentDetails(static_cast<UBillboardComponent *>(Component), Engine);
    }
...
bool FPropertyWindow::AddComponentToActor(AActor *SelectedActor, UClass *ComponentClass, const char *BaseName)
{
    ...
    const FString ComponentName = BuildUniqueComponentName(
        SelectedActor, (BaseName && BaseName[0] != '\0') ? FString(BaseName) : ComponentClass->GetName());

    UActorComponent *NewComponent =
        static_cast<UActorComponent *>(FObjectFactory::ConstructObject(ComponentClass, SelectedActor, ComponentName));
    if (!NewComponent)
    {
        return false;
    }
...
bool FPropertyWindow::DrawAddComponentButton(AActor *SelectedActor)
{
    ...
    if (ImGui::BeginPopup("##AddComponentPopup"))
    {
        ...
        for (const FComponentAddOption &Option : GComponentAddOptions)
        {
            ...
            const bool bCanAdd = (OptionClass != nullptr);
...
void FPropertyWindow::DrawSceneComponentNode(USceneComponent *Component, int32 Depth)
{
    ...
    const bool bOpen = ImGui::TreeNodeEx(Component, Flags, "%s%s (%s)", bIsRoot ? "[Root] " : "", ComponentName.c_str(),
                                         ClassName.c_str());
            ...
            if (UStaticMeshComponent *MeshComp = SelectedActor->GetComponentByClass<UStaticMeshComponent>())
            {
                ...
                if (ImGui::CollapsingHeader("Materials", ImGuiTreeNodeFlags_DefaultOpen))
                {
                    ...
                    if (UStaticMesh *MeshData = MeshComp->GetStaticMesh())
                    {
                        ...
                        ImGui::PushItemWidth(180.f);
                        // DragFloat2를 사용하므로 MasterScroll[0], MasterScroll[1] 값만 조작됩니다. (나머지 2개는 패딩
                        // 역할)
                        if (ImGui::DragFloat2("Scroll All Sections", MasterScroll, 0.001f, -5.0f, 5.0f, "%.2f"))
                        {
                            for (uint32 j = 0; j < NumSections; ++j)
